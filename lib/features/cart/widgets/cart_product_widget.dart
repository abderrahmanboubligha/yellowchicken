        ...
        if (cart!.variations![index].contains(true)) {
          ...
          for (int i = 0; i < cart!.variations![index].length; i++) {
            if (cart!.variations![index][i]!) {
              variationText +=
                  '${variationText.endsWith('(') ? '' : ', '}${variationList?[index].variationValues?[i].level} - ${PriceConverterHelper.convertPrice(variationList?[index].variationValues?[i].optionPrice ?? 0)}';
            }
          }
          variationText += ')';
        }
                            ...
                            Stack(children: [
                              ClipRRect(
                                ...
                                child: CustomImageWidget(
                                  ...
                                  width: 80,
                                  image:
                                      '${splashProvider.baseUrls!.productImageUrl}/${cart!.product!.image}',
                                ),
                              ),
    ...
    return Padding(
        ...
        child: Row(
          ...
          children: [
            ...
            InkWell(
              onTap: () {
                int quantity = cart != null && cart!.product != null
                    ? cartProvider.getCartProductQuantityCount(cart!.product!)
                    : 0;
                couponProvider.removeCouponData(true);
                if (productProvider.checkStock(cart!.product!,
                    quantity: quantity)) {
                  cartProvider.setQuantity(
                      isIncrement: true,
                      fromProductView: false,
