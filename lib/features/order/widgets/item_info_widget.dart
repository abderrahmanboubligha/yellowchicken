    ...
    return Column(children: [
      ListView.builder(
        ...
        itemBuilder: (context, index) {
          List<AddOns> addOns = [];
          List<AddOns>? addons =
              orderProvider.orderDetails![index].productDetails == null
                  ? []
          ...
          if (orderProvider.orderDetails![index].variations != null &&
              orderProvider.orderDetails![index].variations!.isNotEmpty) {
            for (Variation variation
                in orderProvider.orderDetails![index].variations!) {
              variationText +=
                  '${variationText.isNotEmpty ? ', ' : ''}${variation.name} (';
              for (VariationValue value in variation.variationValues!) {
                variationText +=
                    '${variationText.endsWith('(') ? '' : ', '}${value.level} - ${value.optionPrice}';
              }
              variationText += ')';
            }
          } else if (orderProvider.orderDetails![index].oldVariations != null &&
              orderProvider.orderDetails![index].oldVariations!.isNotEmpty) {
            variationText =
                orderProvider.orderDetails![index].oldVariations![0].type ?? '';
          }
                  ...
                  Row(children: [
                        ...
                        child: Column(
                            ...
                            children: [
                              Row(children: [
                                Expanded(
                                    child: Text(
                                  orderProvider.orderDetails![index]
                                      .productDetails!.name!,
                                  style: rubikSemiBold,
                                  maxLines: 2,
                    ...
                    Expanded(
                        child: Column(
                          ...
                          orderProvider
                                      .orderDetails![index].discountOnProduct! >
                                  0
                              ? CustomDirectionalityWidget(
                                  child: Text(
                                  PriceConverterHelper.convertPrice(
                              ...
                              itemBuilder: (context, i) {
                                return Padding(
                                  ...
                                  child: Row(children: [
                                    ...
                                    const SizedBox(width: 2),
                                    Text(
                                        '(${orderProvider.orderDetails![index].addOnQtys![i]})',
                                        style: rubikRegular.copyWith(
                                          color: Theme.of(context).hintColor,
                                        )),
                                  ]),
      ...
      if (orderProvider.trackModel?.isCutlery ?? false)
        Row(mainAxisAlignment: MainAxisAlignment.spaceBetween, children: [
          Text(getTranslated('cutlery', context)!,
