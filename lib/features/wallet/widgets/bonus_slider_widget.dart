    ...
    return Container(
      ...
      child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
        ...
        if(walletBonusModel.endDate != null)
          Text('${getTranslated('valid_till', context)} ${DateConverterHelper.estimatedDate(walletBonusModel.endDate!)}', style: rubikRegular.copyWith(
          fontSize: Dimensions.fontSizeSmall,
        )),
        const SizedBox(height: Dimensions.paddingSizeExtraSmall),

        Text(walletBonusModel.minimumAddAmount != null && walletBonusModel.minimumAddAmount! > 0
            ? '${getTranslated('add_minimum', context)} ${PriceConverterHelper.convertPrice(walletBonusModel.minimumAddAmount)} ${getTranslated('and_enjoy_bonus', context)} '
            '${
            walletBonusModel.bonusType == 'percentage'
                ? '${walletBonusModel.bonusAmount} %'
                : PriceConverterHelper.convertPrice(walletBonusModel.bonusAmount)}'
