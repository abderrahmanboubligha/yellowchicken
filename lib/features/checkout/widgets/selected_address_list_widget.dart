...
class SelectedAddressListWidget extends StatefulWidget {
  ...
  @override
  State<SelectedAddressListWidget> createState() =>
      _SelectedAddressListWidgetState();
}
...
class _SelectedAddressListWidgetState extends State<SelectedAddressListWidget> {
  ...
  void initState() {
    ...
    selectedAddress = widget.isFromAppbar
  ...
  Future<void> _onActionCurrentLocation(
      BuildContext context, bool isLoggedIn, UserInfoModel? userInfo) async {
    final CheckoutProvider checkoutProvider =
        Provider.of<CheckoutProvider>(context, listen: false);
    final SplashProvider splashProvider =
        Provider.of<SplashProvider>(context, listen: false);
    ...
    if (widget.isFromAppbar) {
      final bool isSuccess = await locationProvider
          .checkPermission(() async => locationProvider.onChangeCurrentAddress(
      ...
      if (isLoggedIn) {
          ...
          if (context.mounted) {
            ...
            if (address != null) {
              await CheckOutHelper.selectDeliveryAddress(
                  isAvailable: CheckOutHelper.isAddressInCoverage(
                      branchProvider.getBranch(), address),
                  splashProvider: splashProvider,
        ...
        RouterHelper.getAddAddressRoute(
          ...
          addressModel: checkoutProvider.selectedAddress?.id == null
              ? checkoutProvider.selectedAddress ?? AddressModel()
              : AddressModel(),
  ...
  Widget build(BuildContext context) {
    return SingleChildScrollView(
      child: Column(mainAxisSize: MainAxisSize.min, children: [
        ...
        CustomAssetImageWidget(Images.selectAddressBottomSheetIcon,
            height: ResponsiveHelper.isMobile()
                ? 120
                : ResponsiveHelper.isTab(context)
                    ? 140
            ...
            return CustomButtonWidget(
              ...
              onTap: () async {
                final isSuccess = await locationProvider.checkPermission(
                  () async {
                    final currentLocation = await locationProvider
...
class _AddressDismissibleWidget extends StatelessWidget {
  ...
  final bool isActive;
  const _AddressDismissibleWidget(
      {required this.child,
  ...
  Widget build(BuildContext context) {
    return !isActive
        ...
        : ClipRRect(
            borderRadius: BorderRadius.circular(
                Dimensions.radiusDefault), // Match the dialog's border radius
            child: Dismissible(
                  ...
                  builder: (context) => Consumer<LocationProvider>(
                    ...
                    return AlertDialog(
                      ...
                      content: Text(getTranslated(
                          'are_you_sure_to_delete_this_address', context)!),
                      actions: [
                        TextButton(
                        ...
                        TextButton(
                          onPressed: locationProvider.isLoading
                              ? null
                              : () async {
                                  await locationProvider.deleteUserAddressByID(
              ...
              background: Container(
                ...
                decoration: BoxDecoration(
                  ...
                  borderRadius: BorderRadius.circular(Dimensions
                      .radiusDefault), // Match the child's border radius
                ),
              ...
              child: Container(
                decoration: BoxDecoration(
                  ...
                  borderRadius: BorderRadius.circular(Dimensions
                      .radiusDefault), // Match the background's border radius
                ),
                child:
                    child, // Ensure the child has the same styling as the background
              ),
            ),
          );
  }
