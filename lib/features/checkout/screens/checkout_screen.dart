...
import 'package:flutter_restaurant/common/widgets/breadcrumb_widget.dart';
...
class _CheckoutScreenState extends State<CheckoutScreen> {
  ...
  Widget build(BuildContext context) {
    ...
    bool kmWiseCharge = CheckOutHelper.isKmWiseCharge(
        deliveryInfoModel: splashProvider.deliveryInfoModel!);
    ...
    return Scaffold(
      ...
      body: _isLoggedIn || splashProvider.configModel!.isGuestCheckout!
          ? Column(children: [
              ...
              if (!isDesktop)
                BreadcrumbWidget(
                    items: BreadcrumbHelper.getCheckoutBreadcrumb()),
                                                                ...
                                                                return Row(
                                                                    ...
                                                                    children: [
                                                                      Radio(
                                                                        ...
                                                                        onChanged:
                                                                            (value) =>
                                                                                checkoutProvider.updateDateSlot(index),
                                                                      ),
                                                                      const SizedBox(
                                                                          width:
                                                  ...
                                                  child: Column(
                                                      ...
                                                      children: [
                                                        CostSummeryWidget(
                                                          kmWiseCharge:
                                                              kmWiseCharge,
                                                          deliveryCharge: checkoutProvider
                                                                      .orderType ==
                                                                  OrderType
                                                                      .takeAway
                                                              ? 0
                    ...
                    return Container(
                      ...
                      child: Column(children: [
                          ...
                          children: [
                            SizedBox(
                                ...
                                child: CostSummeryWidget(
                                  kmWiseCharge: kmWiseCharge,
                                  deliveryCharge: checkoutProvider.orderType ==
                                          OrderType.takeAway
                                      ? 0
                                      : checkoutProvider.deliveryCharge,
                        ...
                        ConfirmButtonWidget(
                          ...
                          deliveryCharge:
                              checkoutProvider.orderType == OrderType.takeAway
                                  ? 0
                                  : checkoutProvider.deliveryCharge,
  ...
  Future<void> _onInitLoad() async {
    ...
    _cartList = [];
    widget.fromCart
        ? _cartList.addAll(cartProvider.cartList)
    ...
    for (var method
        in splashProvider.configModel?.activePaymentMethodList ?? []) {
    ...
    if (_isLoggedIn || (splashProvider.configModel?.isGuestCheckout ?? false)) {
      ...
      AddressModel? lastDeliveryAddress;
      AddressModel? selectedDeliveryAddress;

      if (_isLoggedIn && locationProvider.currentAddress != null) {
        lastDeliveryAddress = locationProvider.currentAddress;
        if (locationProvider.currentAddress?.id == null) {
          lastDeliveryAddress = locationProvider.currentAddress?.copyWith(
            contactName:
                '${profileProvider.userInfoModel?.fName} ${profileProvider.userInfoModel?.lName}',
            contactPhoneNumber: profileProvider.userInfoModel?.phone,
          );
        }
      } else if (!_isLoggedIn && locationProvider.currentAddress != null) {
        lastDeliveryAddress = locationProvider.currentAddress;
      } else if (_isLoggedIn) {
        lastDeliveryAddress = await locationProvider.getLastDeliveryAddress();
      }

      selectedDeliveryAddress = await CheckOutHelper.selectDeliveryAddressAuto(
          orderType: checkoutProvider.orderType,
          isLoggedIn: (_isLoggedIn || isGuestCheckout),
          lastAddress: lastDeliveryAddress,
      ...
      deliveryCharge = CheckOutHelper.getDeliveryCharge(
        ...
        minimumDistanceForFreeDelivery: splashProvider.deliveryInfoModel
                ?.deliveryChargeSetup?.minimumDistanceForFreeDelivery
                ?.toDouble() ??
            0,
        shippingPerKm: splashProvider.deliveryInfoModel?.deliveryChargeSetup
                ?.deliveryChargePerKilometer
                ?.toDouble() ??
            0,
        minShippingCharge: splashProvider
                .deliveryInfoModel?.deliveryChargeSetup?.minimumDeliveryCharge
                ?.toDouble() ??
            0,
        defaultDeliveryCharge: splashProvider
                .deliveryInfoModel?.deliveryChargeSetup?.fixedDeliveryCharge
                ?.toDouble() ??
            0,
        isTakeAway: checkoutProvider.orderType == OrderType.takeAway,
        kmWiseCharge: splashProvider
                .deliveryInfoModel?.deliveryChargeSetup?.deliveryChargeType ==
            'distance',
        isFreeDelivery: widget.isFreeDelivery,
      );

      checkoutProvider.setDeliveryCharge(
          deliveryCharge: deliveryCharge, isUpdate: true);
      ...
      if ((selectedDeliveryAddress?.contactPersonName == null ||
              selectedDeliveryAddress?.contactPersonNumber == null) &&
          mounted) {
        showCustomSnackBarHelper(
            getTranslated("add_contact_info_to_place_order", context),
            snackBarStatus: SnackBarStatus.info);
      } else if ((selectedDeliveryAddress?.address == null) && mounted) {
        showCustomSnackBarHelper(
            getTranslated("update_location_to_place_order", context),
            snackBarStatus: SnackBarStatus.info);
      }

      CheckOutHelper.selectPaymentMethodAutomatically(
        activePaymentMethodList:
            splashProvider.configModel?.activePaymentMethodList ?? [],
        isCashOnDeliveryActive:
            splashProvider.configModel?.cashOnDelivery ?? false,
      );
    }
  }
